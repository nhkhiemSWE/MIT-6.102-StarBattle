...
import { solvedPuzzleParser } from './Parser';
import { Puzzle, Region, Cell, Mark, PuzzleStatus } from './Puzzle';
...
const puzzleADT: Puzzle = solvedPuzzleParser(input);
 ...
 * @param col, col of the board
 */
export function removeStar(canvas: HTMLCanvasElement, row: number, col: number): void {
    const context = canvas.getContext('2d');
    assert(context, 'unable to get canvas drawing context');
    // save original context settings before we translate and change colors
    context.save();
 ...
 */
function colorDataToHex(r: number, g: number, b: number): string{
 ...
 * @param col
 */
export function drawPoint(canvas: HTMLCanvasElement, row: number, col: number): void {
    const context = canvas.getContext('2d');
    assert(context, 'unable to get canvas drawing context');
    // save original context settings before we translate and change colors
    context.save();
 ...
 * @param canvas
 */
export function drawGrid(canvas: HTMLCanvasElement): void{
    const context = canvas.getContext('2d');
    assert(context, 'unable to get canvas drawing context');
 ...
 * @param color
 */
function highlightRegion(canvas: HTMLCanvasElement, region: Region, color: string): void {
    ...
    for (const r of region.getCells()){
        ...
        context.fillRect(cellCol, cellRow, cellSize, cellSize);
    }
}
 ...
 */
function getNeighbors(c: Cell): Array<Cell>{
    const lowerBound = 1;
    const upperBound = 10;
    const neighbors: Array<Cell> = [];

    if (c.getColumn() > lowerBound) {
        neighbors.push(new Cell(c.getRow(), c.getColumn() - 1, Mark.Empty));
    }
    if (c.getColumn() < upperBound) {
        neighbors.push(new Cell(c.getRow(), c.getColumn() + 1, Mark.Empty));
    }
    if (c.getRow() > lowerBound) {
        neighbors.push(new Cell(c.getRow() - 1, c.getColumn(), Mark.Empty));
    }
 ...
 */
export function center(value: number): number{
 ...
 */
export function colorAllRegions(): void{
